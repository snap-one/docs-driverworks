...
do	--Static lookup tables
	APIBase = 'https://jsonplaceholder.typicode.com'

	TheCatAPIKey = 'MTk4ODI1'
end
...
--[[
	Default function called with returned data from the C4:urlXXX calls (no callback function specified).
	Uses the gloabl ETag table to return cached data if the remote server uses ETags.
	Converts JSON encoded data into Lua tables for callback functions to use.
	Pulls ticket information from GlobalTicketHandlers table to get correct context and callback function to send data to.
--]]
...
--[[
	Main worker function for the urlXXX calls.
	Inserts appropriate context and information on callback functions into the GlobalTicketHandlers table for later retrieval in ReceivedAsync
	Other urlXXX calls are shims for specific HTTP verbs into this function.  If a Driverworks C4:urlXXX call exists for that verb, it is used, otherwise C4:urlCustom is called.
--]]
	local info = {}
...
function Navigator:urlDo (idBinding, seq, method, url, data, headers, callback, context)
--[[
	Main worker function for the Navigator:urlXXX calls.
	Uses a local function as callback to allow for specific Navigator object response using navId and seq.
	Other Navigator:urlXXX calls are shims for specific HTTP verbs into this function.
--]]
	...
	urlDo (method, url, data, headers, navTicketHandler, context)
end

function Navigator:urlGet (idBinding, seq, url, headers, callback, context)
	self:urlDo (idBinding, seq, 'GET', url, data, headers, callback, context)
end

function Navigator:urlPost (idBinding, seq, url, data, headers, callback, context)
	self:urlDo (idBinding, seq, 'POST', url, data, headers, callback, context)
end

function Navigator:urlPut (idBinding, seq, url, data, headers, callback, context)
	self:urlDo (idBinding, seq, 'PUT', url, data, headers, callback, context)
end

function Navigator:urlDelete (idBinding, seq, url, headers, callback, context)
	self:urlDo (idBinding, seq, 'DELETE', url, data, headers, callback, context)
end

function Navigator:urlCustom (url, method, data, headers, callback, context)
	self:urlDo (idBinding, seq, method, url, data, headers, callback, context)
end
...
function Navigator:Browse (idBinding, seq, args)
	...
	-- we're hard coding these values for now as the placeholder API can return up to 5000 items, which we don't want to process!
	local offset = 0
	local limit = 20
	...
	if (id == nil and itemType == nil) then	-- this should only happen when someone presses a tab, so check the tabId for which screen to generate
		if (tabId == 'Users') then
			url = MakeURL ('/users', {_start = offset, _limit = limit})

		elseif (tabId == 'Albums') then
			url = MakeURL ('/albums', {_start = offset, _limit = limit})

		elseif (tabId == 'Photos') then
			url = MakeURL ('/photos', {_start = offset, _limit = limit})
		end

	elseif (itemType == 'user' and screenId == 'AlbumsScreen') then
		url = MakeURL ('/albums', {userId = id, _start = offset, _limit = limit})

	elseif (itemType == 'album' and screenId == 'PhotosScreen') then
		url = MakeURL ('/photos', {albumId = id, _start = offset, _limit = limit})
	end
	...
	elseif (url) then
		self:urlGet (idBinding, seq, url, {}, 'BrowseResponse', args)
	end
end

function Navigator:BrowseResponse (idBinding, seq, strError, responseCode, tHeaders, data, context, url)
	local responseType = string.match (url, APIBase .. '/(%w+)')

	local response = {}

	if (responseType == 'users') then
		for _, user in ipairs (data) do
			...
			item.subtitle = user.company and user.company.name

			item.default_action = 'SelectItem'
			item.link = true

			table.insert (response, item)
		end

	elseif (responseType == 'albums') then
	...
	elseif (responseType == 'photos') then
		for _, photo in ipairs (data) do
			...
			item.albumId = photo.albumId

			item.image_list = photo.url
